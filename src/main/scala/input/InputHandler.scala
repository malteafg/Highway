...
object InputHandler {
    ...
    def keyPressed(window: Long, key: Int, scancode: Int, action: Int, mods: Int): Unit = {
        ...
        Options.log(s"Key \'${if(key.toChar.isLetterOrDigit) key.toChar else key}\' was ${if(isPressed(event)) "pressed" else if(isReleased(event)) "released" else if}", Options.Keys)
    ...
    def mousePressed(window: Long, button: Int, action: Int, mods: Int): Unit = {
        ...
        Options.log(s"Button ${button} was ${if(isPressed(event)) "pressed" else "released"} on (${mousePos.x}, ${mousePos.y})", Options.MousePressed)
